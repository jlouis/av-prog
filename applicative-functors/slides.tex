...
eval (Var x) \gamma = fetch x \gamma
eval (Val i) \gamma = i
eval (Add p q) \gamma = eval p \gamma + eval q \gamma
\end{code}
...
eval (Add p q) = \mathbb{K} (+) '\mathbb{S}' eval p '\mathbb{S}'eval q
\end{code}
where
\begin{code}
\mathbb{K} :: a \righarrow env \rightarrow a
\mathbb{K} x \gamma = x
\end{code}
and
\begin{code}
...
Rules:\\
identity: pure id \oplus u = u\\
composition pure (\dot) \oplus u \oplus v \oplus w =  u \oplus (v \oplus w)
homomorphism pure f \oplus pure x = pure (f x)
interchange u \oplus  
\end{frame}
